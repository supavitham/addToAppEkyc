...
import 'package:add_to_app_flutter_module/widgets/circularProgress.dart';
...
class CameraScanIDCard extends StatefulWidget {
  ...
  @override
  _CameraScanIDCardState createState() => _CameraScanIDCardState();
}
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            circularProgress(),
            SizedBox(height: 5),
            Text('${'System_is_processing'.tr}...', style: TextStyle(color: Colors.white)),
          ],
        ),
      );
    } else {
  ...
  _onCaptureBackPressed() async {
    ...
    try {
      ...
      if (resBack.isNotEmpty) {
        ocrBackLaser = resBack['laser_number'].replaceAll(new RegExp("-"), "");
        ...
        var verifyDOPA = await PostAPI().call(
          url: '$hostRegister/users/verify_dopa',
  ...
  _onCaptureNoOCR() async {
    try {
      ...
      await _controller!.takePicture().then((v) {
        Navigator.pop(this.context, v.path);
      });
    } catch (e) {}
  }
}
